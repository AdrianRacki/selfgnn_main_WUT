...
def from_rdmol(mol: Any) -> "torch_geometric.data.Data": # type: ignore
    ...
    for atom in mol.GetAtoms():  # type: ignore
        ...
        row.append(atom.GetTotalValence())
    ...
    x = torch.tensor(xs, dtype=torch.long).view(-1, 9)  # 2nd number in view is node dim
    ...
    for bond in mol.GetBonds():  # type: ignore
        ...
        e.append(e_map["is_in_ring"].index(bond.IsInRing()))
